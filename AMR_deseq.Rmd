...
Here, I need to combine the deseq results with the annotations from RGI (Resistance Gene Identifier using the Comprehensive Antibiotic Resistance Database (CARD)). RGI was run on hipergator because the fasta file was too large for the web-based tool. 
...
rgi <- read.table("rgi_out.txt",sep="\t",header=TRUE,quote="")
rgi_counts <-merge(rgi,counts,by="transcript_id")


# remove disease samples to compare just before and after antibiotic treatment
meta2 <-meta[meta$type != "disease",]
rgi_counts2 <- rgi_counts
rgi_counts2$AMR1D <- NULL
rgi_counts2$AMR2D <- NULL
rgi_counts2$AMR3D <- NULL
rgi_counts2$AMR5D <- NULL
rgi_counts2$AMR7D <- NULL
rgi_counts2$AMR8D <- NULL
rgi_counts2$AMR9D <- NULL
